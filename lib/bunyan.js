 ...
 */
function getCaller3Info(depth = 3) {
    if (this === undefined) {
...
function Logger(options, _childOptions, _childSimple) {
    ...
    if (options.srcDepth) {
        this.srcDepth = options.srcDepth
    }
    xxx('Logger: ', self)
...
function mkRecord(log, minLevel, args) {
    ...
    if (log.src && !rec.src) {
        rec.src = getCaller3Info(log.srcDepth)
    }
    rec.v = LOG_VERSION;
